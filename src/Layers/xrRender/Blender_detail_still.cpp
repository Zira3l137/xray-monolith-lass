...
void CBlender_Detail_Still::Compile(CBlender_Compile& C)
{
	...
	case SE_R2_NORMAL_HQ: // deffer wave
		uber_deffer(C, false, "detail_w", "base", true, 0, false, false, 200);
		break;
	case SE_R2_NORMAL_LQ: // deffer still
		uber_deffer(C, false, "detail_s", "base", true, 0, false, false, 200);
		break;
	}
}
#else
...
void CBlender_Detail_Still::Compile(CBlender_Compile& C)
{
	...
	case SE_R2_NORMAL_HQ: // deffer wave
		if (bUseATOC)
		{
			uber_deffer(C, false, "detail_w", "base_atoc", true, 0, true, false, 200);
		...
		uber_deffer(C, false, "detail_w", "base", true, 0, true, false, 200);
		C.r_Stencil(TRUE, D3DCMP_ALWAYS, 0xff, 0x7f, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);
		C.r_StencilRef(0x01);
		C.r_CullMode(D3DCULL_NONE);
		if (bUseATOC)
	...
	case SE_R2_NORMAL_LQ: // deffer still
		if (bUseATOC)
		{
			uber_deffer(C, false, "detail_s", "base_atoc", true, 0, true, false, 200);
			C.r_Stencil(TRUE, D3DCMP_ALWAYS, 0xff, 0x7f, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);
			C.r_StencilRef(0x01);
			C.r_CullMode(D3DCULL_NONE);
			C.r_ColorWriteEnable(false, false, false, false);
		...
		uber_deffer(C, false, "detail_s", "base", true, 0, true, false, 200);
		C.r_Stencil(TRUE, D3DCMP_ALWAYS, 0xff, 0x7f, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);
		C.r_StencilRef(0x01);
		C.r_CullMode(D3DCULL_NONE);
		//	Need this for ATOC
